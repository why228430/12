...
def demo():
    net = dca_cbam_resnet50(num_classes=1000)
    y = net(torch.randn(10, 3, 224, 224))
    print(y.size())


def dca_cbam_resnet18(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 18, 1000)
    return model


def dca_cbam_resnet34(pretrained=False, **kwargs):
    model = ResidualNet('ImageNet', 34, 1000)
    return model


def dca_cbam_resnet101(pretrained=False, **kwargs):
